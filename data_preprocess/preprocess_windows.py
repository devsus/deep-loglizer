...
params ={
    ...
    "label_file": "../data/Windows/anomaly_label.csv",
...
def load_Windows(
    ...
    label_data_dict = dict(zip(label_data["EventId"], label_data["Label"].astype(int)))
    struct_log["Label"] = struct_log["EventId"].map(label_data_dict).fillna(0).astype(int)

    struct_log["time"] = pd.to_datetime(
        struct_log["Date"] + " " + struct_log["Time"], format="%Y-%m-%d %H:%M:%S"
    )
    struct_log["seconds_since"] = (
        (struct_log["time"] - struct_log["time"][0]).dt.total_seconds().astype(int)
    )

    # build sessions by time range (BGL logic)
